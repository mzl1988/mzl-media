...
module.exports = ->
  if config.sogou.start
    search()

search = ->
  Promise.resolve()
  .then ->
    WechatDB.findGzh(config.sogou.offset, config.sogou.limit)
  .then (wechats)->
    _.reduce wechats, (promise, wechat, i) ->
      promise.then ->
        log "#{++i} [#{wechat.wechat_name}]"
        createUrls(wechat.wechat_name)
      .delay 1000 * 5
    , Promise.resolve()
  .catch (error) ->
